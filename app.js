...
app.put('/posts/:postId', requireAuth, (req, res) => {
    const postId = req.params.postId;
    const { text } = req.body;

    Post.findOneAndUpdate({ _id: postId, userId: req.user.userId }, { text }, { new: true }, (err, post) => {
        if (err) return res.status(500).json({ message: 'Internal Server Error' });
...
app.delete('/posts/:postId', requireAuth, (req, res) => {
    ...
    Post.findOneAndDelete({ _id: postId, userId: req.user.userId }, (err, post) => {
        if (err) return res.status(500).json({ message: 'Internal Server Error' });
        if (!post) return res.status(404).json({ message: 'Post not found' });
